...
CameraDataProcessing::CameraDataProcessing() {
    ...
    mAnnotationImage->create(512, 424, fast::TYPE_UINT8, 1);
...
void CameraDataProcessing::execute() {
    ...
    if (mTargetCloudExtracted) {
        ...
        auto icp = fast::IterativeClosestPoint::New();
...
void CameraDataProcessing::addLine(Eigen::Vector2i start, Eigen::Vector2i end) {
    std::cout << "Drawing from: " << start.transpose() << " to " << end.transpose() << std::endl;
    // Draw line in some auxillary image
    mAnnotationImage = mAnnotationImage->copy(fast::Host::getInstance());
    fast::ImageAccess::pointer access = mAnnotationImage->getImageAccess(ACCESS_READ_WRITE);
    Eigen::Vector2f direction = end.cast<float>() - start.cast<float>();
    ...
    for (int i = 0; i < length; ++i) {
        ...
        for (int a = -brushSize; a <= brushSize; a++) {
            for (int b = -brushSize; b <= brushSize; b++) {
                Eigen::Vector2f offset(a, b);
                if (offset.norm() > brushSize)
                    continue;
                Eigen::Vector2f position = start.cast<float>() + direction * distance + offset;
                try {
...
void CameraDataProcessing::calculateTargetCloud(RealSenseStreamer::pointer streamer) {
    ...
    auto access = mAnnotationImage->getImageAccess(ACCESS_READ);
    auto meshAccess = mCurrentCloud->getMeshAccess(ACCESS_READ);
    ...
    for (int y = 0; y < mAnnotationImage->getHeight(); ++y) {
        for (int x = 0; x < mAnnotationImage->getWidth(); ++x) {
            try {
                if (access->getScalar(Eigen::Vector2i(x, y)) == 1) {
...
void CameraDataProcessing::setMaxRange(float range) {
    if(range < 0)
        throw fast::Exception("Range has to be >= 0");
    mMaxRange = range;
}
